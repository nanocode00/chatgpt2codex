...
export function registerTools(server: unknown, ctx: ToolContext): void {
        ...
        makeResult(
          {
            ...
            workflow: [
              ...
              "For built-in adapter features, use adapter_gateway mode=catalog when you need to discover safe operation ids and input descriptors, then invoke the exact static operation id. Custom GPT Actions encode operation arguments in argumentsJson as a JSON object string; underlying MCP/internal callers continue to use the native arguments object. If the operation id is already known in the current chat, invoke it directly; catalog is not required before every call. adapter_gateway never forwards arbitrary MCP tool names, handlers, commands, executables, argv, env, modules, or caller-provided capabilities.",
              ...
              "After git_publish mode=create_pr, use git_pr mode=inspect for reviewable PR state. git_pr mode=approve and mode=request_changes require the exact inspected head SHA; request_changes also requires a review body. Use git_pr mode=merge only when the user explicitly authorizes merging. Merge requires the exact inspected head SHA and never uses force/admin/auto-merge or branch deletion.",
  ...
  registerTool(
    "git_pr",
    {
      title: "Inspect, review, or safely merge a GitHub PR",
      description: "Inspect PR state read-only, submit approve/request-changes reviews against an exact inspected head SHA, or merge with an exact-head concurrency guard and existing remote authorization policy.",
      annotations: COMMAND_RUN_ANNOTATIONS,
