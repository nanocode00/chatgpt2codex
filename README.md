...
-> git_pr(inspect)
-> git_pr(diff)
-> git_pr(approve | request_changes)
-> git_pr(merge)
```

`git_workspace`, `git_publish`, and `git_pr` use fixed Git/GitHub operations rather than arbitrary commands. The remote is fixed to `origin`, force push and arbitrary refspecs are not supported, and credentials are never accepted as Action inputs. For local sync, call `git_workspace(fetch)` first and then `git_workspace(fast_forward)`: fast-forward updates only the current clean branch to exactly `origin/<current-branch>`, requires a matching upstream, and rejects ahead/diverged histories without reset, rebase, pull, or merge commits. GitHub PR creation/inspection/diff/review/merge requires an installed, already-authenticated GitHub CLI (`gh`). `git_pr(inspect)` retries only transient GitHub mergeability calculation (`UNKNOWN`) with a fixed 1s/2s/4s backoff and at most four total views. `git_pr(diff)` reads the PR patch directly from GitHub and never switches branches or mutates the local working tree, so existing uncommitted work is not a reason to skip code review. PR reviews require the exact inspected head SHA and are submitted through GitHub's review API with that SHA as `commit_id`; `request_changes` also requires a non-empty review body. PR merge requires an explicit user request and the exact inspected head SHA; pending or failing checks and requested changes are rejected. Merge commands themselves are never retried. Admin/force/auto-merge bypasses and automatic branch deletion are not supported.
...
Built-in adapter expansion no longer consumes new Custom GPT Action operationIds. SQLite, Python/Notebook runtime operations, Docker, future PostgreSQL, Redis, and similar built-in adapters add static safe operations under `adapter_gateway` rather than adding dedicated OpenAPI operations. `catalog` returns only bounded operation metadata (id, adapter, capability, symbolic availability, description, safe input descriptor); profile payloads, paths, executable locations, DSNs, environment values, secrets, handler/module paths, and raw validator internals remain server-side. Custom GPT Actions encode invoke arguments in `argumentsJson`, for example `{ "mode": "invoke", "projectId": "proj", "operation": "docker.logs", "argumentsJson": "{\"profile\":\"local\",\"service\":\"api\",\"lines\":20}" }`; the HTTP bridge parses that string and forwards the native `arguments` object to the unchanged internal/MCP gateway contract. Direct internal/MCP callers continue to use native JSON `arguments` objects. The selected static operation's own strict validator still rejects unknown fields and enforces semantic bounds before authorization and handler execution.
...
Safe npm dependency management v1 is available through `npm.inspect`, `npm.install`, and `npm.remove`. `npm.inspect` is a read operation that returns only bounded `dependencies`/`devDependencies` package names and version specs plus package/lockfile presence; it does not return scripts, npm config, credentials, environment values, or project paths. `npm.install` and `npm.remove` are write-capability, `remote-exec` operations and therefore require `REMOTE_EXEC=1`, `REMOTE_WRITE=1`, and a full-write project lease for remote use. They accept only conservative npm registry package specs (`name`, `@scope/name`, exact `x.y.z`, or caret `^x.y.z` forms), reject URL/git/file/link/workspace/tarball/alias/local-path inputs and option-like names, and never accept caller executable, argv, cwd, env, registry, prefix, workspace, global, script, lockfile, audit, or timeout flags. Invocations are project-local with server-owned argv and `shell:false`; install/remove always include `--ignore-scripts --no-audit --no-fund`, with `--save-dev` added only for an explicit `dev:true` install. Safe npm operations intentionally install packages with lifecycle scripts disabled. Package retrieval remains an open-world network operation; safety comes from layered authorization, static handlers, conservative registry-only specs, fixed arguments, project confinement, disabled lifecycle scripts, bounded execution/output, and sanitized results/errors—not from assuming downloaded packages are trusted.

PostgreSQL read-only v1 is available through `postgres.profiles`, `postgres.inspect`, and `postgres.query`. Operators configure `CHATGPT2CODEX_POSTGRES_PROFILES` with an absolute trusted `projectRoot`, a `connectionStringEnv` name, and optional user-schema allowlist (default `public`); the profile stores no DSN, and model-facing results expose only aliases and bounded metadata/query results. Each operation verifies the canonical selected project root before reading the DSN from the named environment variable, accepts only `postgres:`/`postgresql:` URLs, opens a fresh bounded-timeout `pg.Client`, starts `BEGIN TRANSACTION READ ONLY`, applies local statement/lock/idle-in-transaction timeouts, verifies `transaction_read_only`, rejects superuser/role-creation/database-creation/replication/BYPASSRLS roles, and always closes the client with best-effort rollback on error. Query SQL uses a PostgreSQL-specific conservative lexical guard and server-side outer `LIMIT`; inspection is fixed and parameterized to allowlisted user schemas. Driver errors and connection details are never returned.

Use a dedicated PostgreSQL read-only login role for these profiles; do not reuse writer, admin, or superuser credentials. Grant only the minimum `SELECT` privileges required, and review role memberships plus any `SET ROLE` capability. Also review sequence privileges, `EXECUTE` on side-effectful or `SECURITY DEFINER` functions, and writable/untrusted schemas reachable through `search_path`. `BEGIN READ ONLY` plus the lexical SQL guard are defense-in-depth, not a complete side-effect sandbox: PostgreSQL grants and role configuration remain the authoritative security boundary.
