...
Adapter operation capabilities are declarative metadata only. They do not dispatch operations and do not replace the existing lease, remote-write, remote-exec, or other safety gates. There is intentionally no generic adapter execution API. Python runtime profiles were the first consumer; SQLite is the first database consumer. Future built-in adapters such as PostgreSQL can reuse the profile framework without turning it into a dynamic extension system.
...
The dedicated `database` Action has flat, importer-compatible inputs and three explicit modes: `list_profiles`, `inspect`, and `query`. Callers provide a project ID and profile alias, never a database path, URL, DSN, executable, argv, environment, or extension. All three modes require only the existing `read` capability, so ordinary calls can auto-select a read-only project lease and do not require `REMOTE_WRITE` or `REMOTE_EXEC`.
...
The public Action counts are now 25 by default, 28 with `REMOTE_EXEC`, 26 with `REMOTE_E2E`, and exactly 30 with both enabled. Because the combined surface has reached the 30-operation budget, another dedicated Action should be preceded by Action consolidation. PostgreSQL, database writes, migrations, SQL-file execution, Docker integration, generic adapter dispatch, and SQLite extension loading are intentionally deferred.
