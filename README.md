...
The public Action counts remain 25 by default, 28 with `REMOTE_EXEC`, 26 with `REMOTE_E2E`, and exactly 30 with both enabled: removing the dedicated `database` Action and adding `adapter_gateway` is a net-zero change. Regression coverage constructs many additional internal safe adapter operations and verifies that the OpenAPI operationId set does not change. PostgreSQL, Docker, Redis, database writes, migrations, SQL-file execution, generic MCP forwarding, dynamic adapter loading, and SQLite extension loading are intentionally deferred.
